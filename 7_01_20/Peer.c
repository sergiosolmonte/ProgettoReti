...
int fd = fdApp;
  ...
  if(choice==3){
    ...
    channels[indexC].stateP = amount1; //salvo mio stato iniziale per il channel
...
void  printPeerList() {
  ...
  for(i=0;i<indexC&&indexC!=0;i++)
    printf(" FD = %d  ID = %c  PORTA = %d STATO = %d\n",channels[i].fd,channels[i].id,channels[i].port,channels[i].stateP);
}
...
void *menu_exec(void* arg){
  ...
  scanf("%d",&key);
  switch (key) {
    case 1: fflush(stdin);pthread_create(&thread_action,NULL,peerConnect,NULL);pthread_join(thread_action,NULL);
    case 2: fflush(stdin);printf("Ecco la lista dei peer\n");Pproto.flag=1;
  }
...
void sendMoney(void *arg){
  ...
  int x,fdX,sendALT=0; //x e' la porta, fdX e' il suo valore hash associato nell'array channels
  scanf(%d,&x);
  fdX=hashcode(x);
