  ...
  while (1) {
    ...
    if (Peer.lastPing == 0) {
      ...
      */
      ...
      i++;
      sendto(sock, &Peer, sizeof(struct ping_protocol), 0,
             (struct sockaddr *)&in, len);
    } else if (Peer.flag == 0) {
      start = clock();
      Peer.lastPing = start;
      ...
      display();
      sendto(sock, &ArrayPeers, sizeof(ArrayPeers), 0, (struct sockaddr *)&in,
             len);
    }
  }
}
  ...
  while (1) {
    ...
    for (int n = 0; n < i; n++) {
      ...
      if (ArrayPeers[n].flag == 4) {
        ...
        total_t = (double)(start - ArrayPeers[n].lastPing) / CLOCKS_PER_SEC;
        // Se il peer non è attivo da 10 secondi viene eliminato
        if (total_t > 10) {
          ArrayPeers[n].name = ' ';
          ArrayPeers[n].rec_port = 0;
          ArrayPeers[n].flag = 4;
        }
      }
    }
